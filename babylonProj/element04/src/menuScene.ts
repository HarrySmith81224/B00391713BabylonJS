...
import {
  ...
  function createSceneButton(scene: Scene, name: string, index: string, x: string, y: string, advtex) {
    let button = GUI.Button.CreateSimpleButton(name, index);
        ...
        const buttonClick = new Sound("MenuClickSFX", "./assets/audio/arcade-kid.mp3", scene, null, {
          loop: true,
          autoplay: false,
        });
  ...
  export default function menuScene(engine: Engine) {
    ...
    let textBG = createRectangle(scene, "300px", "100px", "600px", "-200px", 20, "white", 4, "purple", advancedTexture);
    let titleText = createText(scene, "THE GAME", "600px", "-200px", "45", "white", advancedTexture);
    let button1 = createSceneButton(scene, "but1", "Start Game", "600px", "-75px", advancedTexture);
    let button2 = createSceneButton(scene, "but2", "Options", "600px", "0px", advancedTexture);
